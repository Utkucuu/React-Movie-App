...
function MinLGArrow() {
  ...
  return (
    <div className="group text-sky-200">
      ...
      <button
        style={{ zIndex: 101 }}
        className=" fixed bottom-10 right-6 z-20  flex flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-t from-cyan-400 to-sky-900 hover:bg-gradient-to-b lg:hidden"
        onClick={() => window.scrollTo(0, 0)}
      >
        <CircleIcon />
        {/* <span className=" absolute top-10 p-1 text-[10px] font-bold text-sky-900 opacity-0 duration-500 group-hover:opacity-70 dark:text-sky-100">
